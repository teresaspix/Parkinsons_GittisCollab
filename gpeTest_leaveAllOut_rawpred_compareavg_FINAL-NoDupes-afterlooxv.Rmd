...
---
```{r}
#This file takes in the first data collected, separated by PV and Lhx6, shuffled and duration not logged
...
#Read in the PV data file, replace the path here with your own path to the file
PVFile <- "~/Documents/original_data/firstTwoDataPV_shuffled_withAverages_notLogged.csv"	
...
#Begin LOOXV with GPR linear kernel
count = 0
 for (row in 1:nrow(XTrainPV)) {
  ...
  #Leave out all duplicate parameter settings
...
#Repeat process above, now with a degree of 2 (polynomial kernel)
...
count = 0
 for (row in 1:nrow(XTrainPV)) {
  ...
  #Hold out duplicate parameter settings
  delIndices = which((XTrainPV$Frequency==curFreq)&
                       (XTrainPV$Duration==curDur)&
                       (XTrainPV$AmpMult==curAmpMult))
  print(delIndices)
...
```{r}
#Write out the predictions into a CSV file for analysis in Python
write.csv(PVData,file="PVData_wGPpredictions.csv",row.names=FALSE)
```


```{r setup, include=FALSE}

#Repeat steps above, now for Lhx6 data, replace the path below with your own path
#########
Lhx6File <- "~/Documents/original_data/firstTwoDataLhx6_shuffled_withAverages_notLogged.csv"	
...
write.csv(Lhx6Data,file="Lhx6Data_wGPpredictions.csv",row.names=FALSE)
```
